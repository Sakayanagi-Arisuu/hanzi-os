...
export const HSK3_PERSONAL_DOMAIN_PACK_RELATIVE_PATH =
...
export const loadHsk3PersonalDomainPackBundle = (
  root = process.cwd(),
) => {
  ...
  const packPath = join(root, HSK3_PERSONAL_DOMAIN_PACK_RELATIVE_PATH);
  return {
    blueprintBundle,
    priorLessonBundle,
    packPath,
...
export const validateHsk3PersonalDomainPackBundle = ({
  blueprintBundle,
  priorLessonBundle,
  pack,
}) => {
  ...
  if (!isRecord(pack) || pack.schemaVersion !== 1) {
    return {
      valid: false,
      errors: ["HSK3 personal domain pack schemaVersion must be 1"],
    };
  }
  if (
    pack.packId !== "hsk3-personal-paragraph-domain-2026.07"
    || pack.level !== 3
    || pack.domainId !== DOMAIN_ID
    || pack.state !== "ai-assisted-content-draft"
    || pack.learnerVisible !== false
    || pack.releaseEligible !== false
  ) {
    errors.push("HSK3 personal domain content must remain learner-hidden");
  }
  if (
    ...
    || pack.source?.priorLessonPackId !== priorLessonBundle.pack.packId
    || pack.source?.priorLessonPackSha256
      !== fileSha256(priorLessonBundle.packPath)
  ) {
    errors.push("HSK3 personal domain source or prior-pack binding is stale");
  }
  if (
    ...
    errors.push("HSK3 personal domain authorship must not imply review");
  }
  if (
    ...
    errors.push("HSK3 personal domain review/audio policy must fail closed");
  }
  if (
    pack.coverageClaims?.remainingPersonalDomainLessonDraftsComplete
      !== true
    || pack.coverageClaims?.personalDomainLessonDraftsCompleteWithPriorPack
      !== true
    ...
    errors.push("HSK3 personal domain coverage claims are invalid");
  }

  const lessons = Array.isArray(pack.lessons) ? pack.lessons : [];
  if (
    lessons.length !== 4
    || !exact(lessons.map((lesson) => lesson.lessonId), LESSON_IDS)
  ) {
    errors.push("HSK3 personal domain pack must contain the remaining 4 lessons");
  }
  ...
  if (
    ...
    errors.push("HSK3 personal domain IDs must be globally unique");
  }
  ...
  const expectedCounts = {
    ...
    personalDomainLessonsWithPriorPack: lessons.length + 1,
  ...
  if (!exact(pack.counts, expectedCounts)) {
    errors.push("HSK3 personal domain summary counts are stale");
  }
  return {
...
export const assertValidHsk3PersonalDomainPackBundle = (bundle) => {
  const result = validateHsk3PersonalDomainPackBundle(bundle);
  if (!result.valid) {
    throw new Error(
      `Invalid HSK3 personal domain pack:\n- ${result.errors.join("\n- ")}`,
    );
  }
  return result;
};
