...
export const assertSafeHsk2ReviewId = (value, label) => {
  if (
    typeof value !== "string"
    || value.length === 0
    || value.length > 128
...
const loadWorkflowContext = (root) => {
  ...
  const manifestBundle = loadHsk2ReviewManifestBundle(root);
  assertValidHsk2ReviewManifestBundle(manifestBundle);
...
const normalizedTargets = (batch) => ({
  ...
  taskIds: uniqueSortedStrings(batch.taskIds),
  topicIds: uniqueSortedStrings(batch.topicIds),
  dialogueIds: uniqueSortedStrings(batch.dialogueIds),
  practiceItemIds: uniqueSortedStrings(batch.practiceItemIds),
  promptIds: uniqueSortedStrings(batch.promptIds),
...
const resolveBatchFromContext = async (workflowContext, batchId) => {
  ...
  if (!manifestBatch) throw new Error(`Unknown HSK2 review batch: ${batchId}`);
...
export const resolveHsk2ReviewBatch = async (
  root = process.cwd(),
  batchId,
) => resolveBatchFromContext(loadWorkflowContext(root), batchId);

export const buildHsk2ReviewAssignmentDocument = async ({
  root = process.cwd(),
  assignmentId,
  batchId,
  role,
  ...
  assertSafeHsk2ReviewId(assignmentId, "assignmentId");
  assertSafeHsk2ReviewId(assignedBy, "assignedBy");
  assertSafeHsk2ReviewId(assignee, "assignee");
  if (
  ...
  const context = await resolveHsk2ReviewBatch(root, batchId);
...
export const validateCompletedHsk2ReviewDocument = async (
  document,
  {
    root = process.cwd(),
    nowEpochMs = Date.now(),
  ...
  for (const [value, label] of [
    ...
    try {
      assertSafeHsk2ReviewId(value, label);
    } catch (error) {
  ...
  let context = null;
  try {
    context = await resolveHsk2ReviewBatch(root, assignment?.batchId);
  } catch (error) {
    errors.push(error instanceof Error ? error.message : String(error));
  }
  if (context) {
...
export const validateHsk2ReviewWorkflow = async (root = process.cwd()) => {
