...
export const assertSafeHsk1ReviewId = (value, label) => {
  if (
    typeof value !== "string"
    || value.length === 0
    || value.length > 128
...
const loadWorkflowContext = (root) => {
  ...
  const manifestBundle = loadHsk1ReviewManifestBundle(root);
  assertValidHsk1ReviewManifestBundle(manifestBundle);
...
const artifactBatches = (artifact) => [
  ...(Array.isArray(artifact.reviewBatches) ? artifact.reviewBatches : []),
  ...(Array.isArray(artifact.packs)
    ? artifact.packs.flatMap((pack) =>
      Array.isArray(pack.reviewBatches) ? pack.reviewBatches : []
    )
    : []),
];

const artifactPracticeItems = (artifact) => [
  ...(Array.isArray(artifact.practiceItems) ? artifact.practiceItems : []),
  ...(Array.isArray(artifact.packs)
    ? artifact.packs.flatMap((pack) =>
      Array.isArray(pack.practiceItems) ? pack.practiceItems : []
    )
    : []),
];

const normalizedTargets = (artifact, batch) => {
  const practiceItemIds = uniqueSortedStrings(batch.practiceItemIds);
  const practiceIdSet = new Set(practiceItemIds);
  const vocabularyIds = uniqueSortedStrings(
    artifactPracticeItems(artifact)
      .filter((item) => practiceIdSet.has(item.itemId))
      .map((item) => item.officialVocabularyId),
  );
  return {
    practiceItemIds,
    vocabularyIds,
    characterIds: uniqueSortedStrings(batch.characterIds),
    grammarRowIds: uniqueSortedStrings(batch.grammarRowIds),
    taskIds: uniqueSortedStrings(
      typeof batch.officialTaskId === "string"
        ? [batch.officialTaskId]
        : [],
    ),
...
const resolveBatchFromContext = async (workflowContext, batchId) => {
  ...
  if (!manifestBatch) throw new Error(`Unknown HSK1 review batch: ${batchId}`);
  ...
  const batch = artifactBatches(loaded.artifact).find(
    (candidate) => candidate.batchId === batchId,
  );
  ...
  const targets = normalizedTargets(loaded.artifact, batch);
...
export const resolveHsk1ReviewBatch = async (
  root = process.cwd(),
  batchId,
) => resolveBatchFromContext(loadWorkflowContext(root), batchId);

export const buildHsk1ReviewAssignmentDocument = async ({
  root = process.cwd(),
  assignmentId,
  batchId,
  role,
  ...
  assertSafeHsk1ReviewId(assignmentId, "assignmentId");
  assertSafeHsk1ReviewId(assignedBy, "assignedBy");
  assertSafeHsk1ReviewId(assignee, "assignee");
  if (
  ...
  const context = await resolveHsk1ReviewBatch(root, batchId);
...
export const validateCompletedHsk1ReviewDocument = async (
  document,
  {
    root = process.cwd(),
    nowEpochMs = Date.now(),
  ...
  for (const [value, label] of [
    ...
    try {
      assertSafeHsk1ReviewId(value, label);
    } catch (error) {
  ...
  let context = null;
  try {
    context = await resolveHsk1ReviewBatch(root, assignment?.batchId);
  } catch (error) {
    errors.push(error instanceof Error ? error.message : String(error));
  }
  if (context) {
...
export const validateHsk1ReviewWorkflow = async (root = process.cwd()) => {
