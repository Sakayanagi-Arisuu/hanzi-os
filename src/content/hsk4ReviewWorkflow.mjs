...
export const assertSafeHsk4ReviewId = (value, label) => {
  if (
    typeof value !== "string"
    || value.length === 0
    || value.length > 128
...
const loadWorkflowContext = (root) => {
  ...
  const manifestBundle = loadHsk4ReviewManifestBundle(root);
  assertValidHsk4ReviewManifestBundle(manifestBundle);
...
const normalizedTargets = (batch) => ({
  ...
  vocabularyIds: uniqueSortedStrings(batch.targetLexemeIds),
  textIds: uniqueSortedStrings(batch.textIds),
  sourceTextIds: uniqueSortedStrings(batch.sourceTextIds),
  grammarRowIds: uniqueSortedStrings(batch.grammarRowIds),
  practiceItemIds: uniqueSortedStrings(batch.practiceItemIds),
  promptIds: uniqueSortedStrings(batch.promptUnitIds),
  rubricIds: uniqueSortedStrings(batch.rubricIds),
...
const audioSignalIsMissing = (value) => {
  if (Array.isArray(value)) {
    return value.some((item) => audioSignalIsMissing(item));
  }
  if (!isRecord(value)) return false;
  const kind = typeof value.kind === "string" ? value.kind : "";
  const modality = typeof value.modality === "string" ? value.modality : "";
  const audioDependent =
    kind.includes("listening")
    || modality.includes("listening")
    || modality.includes("recorded")
    || value.primarySkill === "listening"
    || value.skill === "listening"
    || typeof value.audioRequirement === "string"
    || typeof value.ttsText === "string";
  if (
  ...
  return Object.values(value).some((item) => audioSignalIsMissing(item));
};

const missingAudioTargets = (artifact, flatTargets) => {
  const targetIds = new Set(flatTargets.map((target) => target.targetId));
  ...
  const visit = (value) => {
    ...
    const ids = TARGET_ID_FIELDS.flatMap((field) =>
      typeof value[field] === "string" ? [value[field]] : []
    );
    for (const id of ids) {
      if (targetIds.has(id) && audioSignalIsMissing(value)) {
        missing.add(id);
      }
    }
...
const resolveBatchFromContext = async (workflowContext, batchId) => {
  ...
  if (!manifestBatch) throw new Error(`Unknown HSK4 review batch: ${batchId}`);
  ...
  return {
    ...
    targets,
    flatTargets,
    missingAudioTargetIds:
      manifestBatch.requiredRoles.includes("audio-rights-reviewer")
        ? missingAudioTargets(loaded.artifact, flatTargets)
        : [],
    targetDigest: await sha256Json(targets),
  };
};

export const resolveHsk4ReviewBatch = async (
  root = process.cwd(),
  batchId,
) => resolveBatchFromContext(loadWorkflowContext(root), batchId);

export const buildHsk4ReviewAssignmentDocument = async ({
  root = process.cwd(),
  assignmentId,
  batchId,
  role,
  ...
  assertSafeHsk4ReviewId(assignmentId, "assignmentId");
  assertSafeHsk4ReviewId(assignedBy, "assignedBy");
  assertSafeHsk4ReviewId(assignee, "assignee");
  if (
  ...
  const context = await resolveHsk4ReviewBatch(root, batchId);
...
export const validateCompletedHsk4ReviewDocument = async (
  document,
  {
    root = process.cwd(),
    nowEpochMs = Date.now(),
  ...
  for (const [value, label] of [
    ...
    try {
      assertSafeHsk4ReviewId(value, label);
    } catch (error) {
  ...
  let context = null;
  try {
    context = await resolveHsk4ReviewBatch(root, assignment?.batchId);
  } catch (error) {
    errors.push(error instanceof Error ? error.message : String(error));
  }
  if (context) {
  ...
  if (
    ...
    if (
      ...
      && (context?.missingAudioTargetIds.length ?? 0) > 0
    ) {
      errors.push(
...
export const validateHsk4ReviewWorkflow = async (root = process.cwd()) => {
