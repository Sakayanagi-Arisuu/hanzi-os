import localBankJson from "../../content/runtime/hsk3-level-check-local.json";
...
type LocalBank = {
  ...
  counts: Record<Hsk3LevelCheckSkill, number> & { items: number };
  items: Array<Omit<Hsk3LevelCheckItem, "activityVersion">>;
};
...
export const HSK3_LEVEL_CHECK_BANK_ID = localBank.bankId;
export const HSK3_LEVEL_CHECK_ITEMS: Hsk3LevelCheckItem[] = localBank.items.map(
  (item) => ({
    ...item,
    activityVersion: `${CONTENT_VERSION}:hsk3-level-check:${item.id}:1`,
  }),
);

if (
  localBank.schemaVersion !== 1
  || localBank.bankId !== "hsk3-level-assessment-2026.07"
  || localBank.formId !== "hsk3-level-form-a"
  ...
  || localBank.counts.items !== 54
  || localBank.counts.listening !== 12
  || localBank.counts.reading !== 12
...
export const HSK3_LEVEL_CHECK_SKILL_COUNTS = {
...
} satisfies Record<Hsk3LevelCheckSkill, number>;
