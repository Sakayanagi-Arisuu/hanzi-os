...
const skillLabels: Record<Hsk1LevelCheckSkill, string> = {
...
const practiceDestinations: Record<Hsk1LevelCheckSkill, {
  lessonId: string;
  label: string;
}> = {
  ...
  grammar: {
    lessonId: "hsk1-time-place-events-01-numbers",
    label: "Củng cố mẫu câu HSK1",
  },
};
...
const validAnswers = (value: unknown): value is Record<string, string> => {
  ...
  return Object.entries(value).every(([itemId, optionId]) => {
    const item = HSK1_LEVEL_CHECK_ITEMS.find((candidate) =>
      candidate.id === itemId
    );
...
const loadResume = (): Resume | null => {
  const raw = readLocalStorage(HSK1_LEVEL_CHECK_SESSION_STORAGE_KEY);
  if (!raw) return null;
  try {
    const value = JSON.parse(raw) as Partial<Resume>;
    const current = HSK1_LEVEL_CHECK_ITEMS[value.index ?? -1];
    ...
    if (
      ...
      || value.formVersion !== HSK1_LEVEL_CHECK_FORM_VERSION
      || value.bankId !== HSK1_LEVEL_CHECK_BANK_ID
      || typeof value.sessionId !== "string"
      || !["intro", "question", "result"].includes(value.phase ?? "")
      || !Number.isInteger(value.index)
      || (value.index ?? -1) < 0
      || (value.index ?? HSK1_LEVEL_CHECK_ITEMS.length)
        >= HSK1_LEVEL_CHECK_ITEMS.length
      || typeof value.checked !== "boolean"
      || !selectedValid
      || !validAnswers(value.answers)
    ) {
      throw new Error("stale HSK1 level-check resume");
    }
    return value as Resume;
  } catch {
    removeLocalStorage(HSK1_LEVEL_CHECK_SESSION_STORAGE_KEY);
    return null;
  }
};

const freshSessionId = () => makeIdempotencyKey("hsk1-level-check-session");
...
export function Hsk1LevelCheckPage() {
  ...
  const current = HSK1_LEVEL_CHECK_ITEMS[index];

  useEffect(() => {
    writeLocalStorage(HSK1_LEVEL_CHECK_SESSION_STORAGE_KEY, JSON.stringify({
      version: 1,
  ...
  const check = () => {
    ...
    actions.recordPracticeEvidence({
      ...
      activityId: `hsk1-level-check:${current.id}`,
      skill: current.skill,
      ...
      metadata: {
        ...
        priorExposure: state.evidence.some((item) =>
          item.activityId === `hsk1-level-check:${current.id}`
        ),
      },
    });
  };
  ...
  const next = () => {
    if (index < HSK1_LEVEL_CHECK_ITEMS.length - 1) {
  ...
  const skillResults = useMemo(() => Object.fromEntries(
    (Object.keys(skillLabels) as Hsk1LevelCheckSkill[]).map((skill) => {
      const items = HSK1_LEVEL_CHECK_ITEMS.filter((item) =>
        item.skill === skill
      );
  ...
  ) as Record<Hsk1LevelCheckSkill, { correct: number; total: number }>, [answers]);
  ...
  if (phase === "result") {
    const totalCorrect = HSK1_LEVEL_CHECK_ITEMS.filter((item) =>
      answers[item.id] === item.correctOptionId
    ).length;
  ...
  const correct = selected === current.correctOptionId;
  const progress = Math.round(((index + 1) / HSK1_LEVEL_CHECK_ITEMS.length) * 100);
  return (
    <div className="assessment-live hsk1-level-check" data-testid="hsk1-level-check-live">
      <header>
      ...
      <footer className={checked ? (correct ? "correct" : "wrong") : ""}>
        ...
        <button className="primary-button" disabled={selected === null} type="button" onClick={checked ? next : check}>
          {checked ? (index === 49 ? "Xem kết quả" : "Câu tiếp theo") : "Xác nhận"}
          <ArrowRight size={17} />
        </button>
      </footer>
    </div>
