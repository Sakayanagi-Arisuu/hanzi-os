...
Tại thời điểm chốt kế hoạch, **T001–T100 đều `PENDING`** và tiến độ Reforge là
`0/100 ACCEPTED`. Khi thực thi, thêm trạng thái hiện hành ngay cuối dòng task theo
...
- **T001 — Chốt hợp đồng sản phẩm Reforge.** **Kết quả:** một north-star có persona, phạm vi Mainland Mandarin HSK0–4, năm tab và ranh giới không sao chép. **DoD/Bằng chứng:** trang giới thiệu trong build dev và product spec dùng cùng tên/khái niệm; ba người đọc độc lập mô tả đúng luồng chính. **Phụ thuộc:** không. **Tuyến:** Core + UX.
- **T002 — Lập ma trận benchmark có nguồn.** **Kết quả:** từng năng lực tham chiếu được phân loại `đã quan sát / store claim / chưa xác minh / ngoài phạm vi`. **DoD/Bằng chứng:** ma trận truy ngược từ feature đến nguồn, task và tiêu chí nghiệm thu; không có claim “giống hệt” thiếu bằng chứng. **Phụ thuộc:** T001. **Tuyến:** Core + Browser.
- **T003 — Chụp baseline repo và hành trình hiện tại.** **Kết quả:** inventory route, component, content package, store, API, role và consumer thật. **DoD/Bằng chứng:** người học cũ vẫn mở được một bài, hoàn thành, ôn và khôi phục; snapshot ID/count được máy kiểm tra. **Phụ thuộc:** T001. **Tuyến:** Core + Browser.
...
- **T010 — Dựng bảng nghiệm thu và quality budget.** **Kết quả:** mỗi task có owner/status/evidence; budget cho lỗi nghiêm trọng, accessibility, performance, content accuracy và data loss. **DoD/Bằng chứng:** CI chặn task thiếu evidence; trang người học không lộ log kỹ thuật; baseline gate được lưu trước khi đổi UI. **Phụ thuộc:** T002–T009. **Tuyến:** Core + Browser.
...
- **T066 — Hoàn thiện custom deck.** **Kết quả:** tạo/đổi tên/thêm item/chọn mode/import lawful/export deck trong phạm vi content đã có quyền. **DoD/Bằng chứng:** deck chạy bằng activity engine, lịch không xung đột main queue và dữ liệu cũ khôi phục được; các bank sau đó chỉ gọi API deck chung. **Phụ thuộc:** T023, T062–T065. **Tuyến:** Core + UX. **An toàn:** import validate/schema-version, không overwrite im lặng.
...
- **T069 — Hiệu chỉnh Thất Trụ và báo cáo tiến độ.** **Kết quả:** coverage/accuracy/confidence/trend hiển thị theo level và kỹ năng, kèm mẫu số và giải thích. **DoD/Bằng chứng:** dữ liệu ít luôn hiện “cần thêm lượt”; drill chỉ tác động đúng trụ; test fixture khớp phép tính công bố. **Phụ thuộc:** T046, T061–T068. **Tuyến:** Core + UX.
...
- **T077 — Xây Practice Zone từ vựng.** **Kết quả:** game ngắn ghép nghĩa, âm–chữ, tốc độ recall và phân loại, tái dùng activity/evidence. **DoD/Bằng chứng:** chơi vui nhưng điểm game tách mastery; reduced-motion và không hẹn giờ đều dùng được; pool không lặp cạn. **Phụ thuộc:** T033–T034, T061, T071, T074. **Tuyến:** Core + UX + Content.
- **T078 — Xây Practice Zone ngữ pháp/câu.** **Kết quả:** sắp câu, sửa lỗi, cloze theo điểm ngữ pháp và thử thách hội thoại. **DoD/Bằng chứng:** item có explanation/provenance, độ khó theo HSK, score không mở khóa level nếu thiếu assessment. **Phụ thuộc:** T035–T040, T061, T071. **Tuyến:** Core + Content + UX.
...
- **T087 — Xây exam engine full-length.** **Kết quả:** timed sections, audio once/replay rule, autosave, resume policy, navigation, accessibility và kết quả chi tiết; mini mock là mode riêng. **DoD/Bằng chứng:** cấu hình mỗi level sinh đúng số câu/thời lượng, refresh không mất bài, hết giờ nộp idempotent và UI không yêu cầu login local. **Phụ thuộc:** T045, T054, T086. **Tuyến:** Core + UX + Browser.
- **T088 — Soạn ngân hàng đề nguyên bản.** **Kết quả:** HSK0 diagnostic và tối thiểu hai form full-length độc lập cho mỗi HSK1–4, có audio, đáp án, distractor, giải thích và psychometric metadata ban đầu. **DoD/Bằng chứng:** blueprint coverage 100%, validator/leak/duplicate check xanh, expert language QA; không quảng bá là đề thi thật. **Phụ thuộc:** T052, T086–T087. **Tuyến:** Content + Core. **[HUMAN] [EXT]** cho ghi âm.
- **T089 — Hoàn thiện placement, level check và báo cáo.** **Kết quả:** placement thích ứng, level check end-to-end, breakdown theo kỹ năng/blueprint và kế hoạch học đề xuất. **DoD/Bằng chứng:** fixture known-level đạt calibration target; kết quả không trộn XP/mastery, mở khóa theo rule T049 và export được. **Phụ thuộc:** T049, T061, T087–T088. **Tuyến:** Core + UX + Content. **[HUMAN]** cho calibration.
