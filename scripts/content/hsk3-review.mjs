...
const localPaths = (root, assignmentId) => {
  assertSafeHsk3ReviewId(assignmentId, "assignment-id");
  const localRoot = join(root, "content", "review", "local", "hsk3");
  return {
...
const commandValidate = async (root) => {
  console.log(JSON.stringify({
    valid: true,
    summary: await validateHsk3ReviewWorkflow(root),
  }, null, 2));
};

const commandList = (root) => {
  const bundle = loadHsk3ReviewManifestBundle(root);
  assertValidHsk3ReviewManifestBundle(bundle);
...
const commandExport = async (root, args) => {
  ...
  const document = await buildHsk3ReviewAssignmentDocument({
    root,
    assignmentId,
  ...
  console.log(formatJson({
    ...
    note:
      "Complete only the response object, then run content:hsk3:review:import.",
  }));
};
...
const commandImport = async (root, args) => {
  ...
  const result = await validateCompletedHsk3ReviewDocument(document, { root });
  if (!result.valid) {
...
const main = async () => {
  ...
  throw new Error(`Unsupported HSK3 review command: ${command}`);
};
