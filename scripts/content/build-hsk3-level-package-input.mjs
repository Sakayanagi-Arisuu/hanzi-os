...
import {
  HSK3_LEVEL_PACKAGE_INPUT_DIRECTORY,
...
const main = async () => {
  ...
  if (process.argv.includes("--check-package")) {
    const result = await validateMaterializedHsk3LevelPackage(root);
  ...
  const outputDirectory = resolve(root, HSK3_LEVEL_PACKAGE_INPUT_DIRECTORY);
  ...
  writeJson(resolve(outputDirectory, "coverage-claims.json"), projected.coverageClaims);
  console.log(JSON.stringify({
    valid: true,
    outputDirectory: HSK3_LEVEL_PACKAGE_INPUT_DIRECTORY,
