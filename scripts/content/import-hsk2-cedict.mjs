...
import {
  buildHsk2VocabularyDraft,
  HSK2_CEDICT_SOURCE_RELATIVE_PATH,
  HSK2_VOCABULARY_DRAFT_RELATIVE_PATH,
  parseCedictText,
  serializeHsk2VocabularyDraft,
  sha256,
...
const descriptor = JSON.parse(readFileSync(
  resolve(root, HSK2_CEDICT_SOURCE_RELATIVE_PATH),
  "utf8",
));
...
const draft = buildHsk2VocabularyDraft({
  descriptor,
  inventory: syllabus.inventory,
  inventorySha256: syllabus.inventorySha256,
  cedictEntries,
});
const serialized = serializeHsk2VocabularyDraft(draft);
const outputPath = resolve(root, HSK2_VOCABULARY_DRAFT_RELATIVE_PATH);
...
if (write) {
  ...
  if (checked !== null && checked !== serialized) {
    throw new Error("Checked HSK2 vocabulary draft is stale; rerun with --write");
  }
}
...
console.log(JSON.stringify({
  ...
  output: HSK2_VOCABULARY_DRAFT_RELATIVE_PATH,
  counts: draft.counts,
}, null, 2));
