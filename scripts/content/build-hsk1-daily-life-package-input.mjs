...
import {
  HSK1_DAILY_LIFE_PACKAGE_INPUT_DIRECTORY,
  loadHsk1DailyLifePackageSources,
  projectHsk1DailyLifePackageInputs,
  validateMaterializedHsk1DailyLifePackage,
} from "../../src/content/hsk1DailyLifePackage.mjs";
...
const main = async () => {
  ...
  if (process.argv.includes("--check-package")) {
    const result = await validateMaterializedHsk1DailyLifePackage(root);
  ...
  const projected = await projectHsk1DailyLifePackageInputs(
  ...
  console.log(JSON.stringify({
    valid: true,
    outputDirectory: HSK1_DAILY_LIFE_PACKAGE_INPUT_DIRECTORY,
