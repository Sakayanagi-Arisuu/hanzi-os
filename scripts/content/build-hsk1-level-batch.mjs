...
const main = async () => {
  ...
  const projected = await projectHsk1LevelBatch(
    loadHsk1LevelBatchSources(root),
  );
  const outputs = [
    [HSK1_LEVEL_REVIEW_RELATIVE_PATH, projected.review],
    [HSK1_LEVEL_CORE_RELATIVE_PATH, projected.core],
  ];
