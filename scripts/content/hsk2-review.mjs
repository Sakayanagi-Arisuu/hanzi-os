...
const localPaths = (root, assignmentId) => {
  assertSafeHsk2ReviewId(assignmentId, "assignment-id");
  const localRoot = join(root, "content", "review", "local", "hsk2");
  return {
...
const commandValidate = async (root) => {
  console.log(JSON.stringify({
    valid: true,
    summary: await validateHsk2ReviewWorkflow(root),
  }, null, 2));
};

const commandList = (root) => {
  const bundle = loadHsk2ReviewManifestBundle(root);
  assertValidHsk2ReviewManifestBundle(bundle);
...
const commandExport = async (root, args) => {
  ...
  const document = await buildHsk2ReviewAssignmentDocument({
    root,
    assignmentId,
  ...
  console.log(formatJson({
    ...
    note:
      "Complete only the response object, then run content:hsk2:review:import.",
  }));
};
...
const commandImport = async (root, args) => {
  ...
  const result = await validateCompletedHsk2ReviewDocument(document, { root });
  if (!result.valid) {
...
const main = async () => {
  ...
  throw new Error(`Unsupported HSK2 review command: ${command}`);
};
