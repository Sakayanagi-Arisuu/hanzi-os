...
import {
  buildHsk4VocabularyDraft,
  HSK4_CEDICT_SOURCE_RELATIVE_PATH,
  HSK4_VOCABULARY_DRAFT_RELATIVE_PATH,
  parseCedictText,
  serializeHsk4VocabularyDraft,
  sha256,
...
const sourceFile = resolve(
  sourceArgIndex >= 0
    ...
    : "tmp/hsk4-cedict/cedict_ts.u8",
);
const archiveFile = resolve(
  archiveArgIndex >= 0
    ? args[archiveArgIndex + 1]
    : "tmp/hsk4-cedict/cc-cedict_0.0~repack20260403.orig.tar.xz",
);
const write = args.includes("--write");
const root = process.cwd();
const descriptor = JSON.parse(readFileSync(
  resolve(root, HSK4_CEDICT_SOURCE_RELATIVE_PATH),
  "utf8",
));
...
const draft = buildHsk4VocabularyDraft({
  descriptor,
  inventory: syllabus.inventory,
  inventorySha256: syllabus.inventorySha256,
  cedictEntries,
});
const serialized = serializeHsk4VocabularyDraft(draft);
const outputPath = resolve(root, HSK4_VOCABULARY_DRAFT_RELATIVE_PATH);
...
if (write) {
  ...
  if (checked !== null && checked !== serialized) {
    throw new Error("Checked HSK4 vocabulary draft is stale; rerun with --write");
  }
}
...
console.log(JSON.stringify({
  ...
  output: HSK4_VOCABULARY_DRAFT_RELATIVE_PATH,
  counts: draft.counts,
}, null, 2));
