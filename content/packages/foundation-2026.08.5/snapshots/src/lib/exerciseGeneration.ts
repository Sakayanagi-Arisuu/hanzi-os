...
const buildExerciseCandidateGroups = (
  ...
  const remaining = exercises.filter((exercise) => !requiredIds.has(exercise.id));
...
export function buildExerciseCatalog(
  ...
  const { required, remaining } = buildExerciseCandidateGroups(
    lesson,
    script,
    random,
  );
...
export function buildExercises(
  ...
  const remaining = shuffleWith(candidates, random);
