...
const makeOptions = (
  ...
  const randomAttemptLimit = Math.max(24, count * 8);
  for (
    let attempt = 0;
    attempt < randomAttemptLimit
      && distractors.length < count - 1
      && candidates.length > 0;
    attempt += 1
  ) {
...
const RELEASED_MEANINGS = RELEASED_VOCABULARY.map((word) => word.meaning);
const RELEASED_PINYIN = RELEASED_VOCABULARY.map((word) => word.pinyin);
...
const buildExerciseCandidateGroups = (
  lesson: Lesson,
  ...
  // Bài nhập môn thanh điệu chỉ được hỏi đúng hai năng lực đã công bố:
  // nhận diện âm/thanh và nghe từ. Các dạng nghĩa, đọc câu và tự viết chữ
  // thuộc những bài sau; đưa chúng vào boot-1 khiến người mới bị kiểm tra kiến
  // thức chưa được dạy dù dữ liệu từ vựng có sẵn trong catalog.
  const lessonScopedExercises = lesson.id === "boot-1"
  ...
  const catalogRemaining = exercises.filter(
    (exercise) => !requiredIds.has(exercise.id),
  );
  return { required, remaining, catalogRemaining };
};

/**
 * Returns every current presentation candidate exactly once. Server forms own
 * selection and ordering; clients may only resolve IDs from that frozen form.
 */
export function buildExerciseCatalog(
  ...
  const { required, catalogRemaining } = buildExerciseCandidateGroups(
    lesson,
    script,
    random,
  );
...
export function buildExercises(
  ...
  const representedSkills = new Set(selected.map((exercise) => exercise.skill));

  // Give every activity skill that this lesson can actually present one slot
  // before randomly filling the rest of the ten-activity session. This changes
  // only future form selection: IDs, versions and the complete catalog stay
  // stable, so saved and server-owned forms remain resolvable.
  for (const exercise of shuffledCandidates) {
    if (selected.length >= 10) break;
    if (representedSkills.has(exercise.skill)) continue;
    selected.push(exercise);
    selectedIds.add(exercise.id);
    representedSkills.add(exercise.skill);
  }
